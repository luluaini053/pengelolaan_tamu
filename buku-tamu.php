...
<body style="background:#428bca;">
	...
	<div class="row">
		...
		<?php
			...
			$query = "SELECT max(id_tamu) as id_tamu FROM buku_tamu";
			$sql = mysql_query($query);
			$row = mysql_fetch_array($sql);
			$idtamu	= $row['id_tamu'];
			$no	= (int) substr($idtamu, 4, 2);
			$no++;
			$idtamu='TAMU'.sprintf("%02s", $no);
			
			$q = mysql_query("SELECT * FROM pelanggan where username = '$_SESSION[nama_pelanggan]'");
			$r = mysql_fetch_array($q);
			if(!empty($submit)){
				 $simpan = mysql_query("INSERT INTO buku_tamu VALUES('$idtamu', '$r[id_pelanggan]', '$keperluan', now())");
				 if($simpan==true) {
