...
<head>
	...
	<title>Pekata - Keluhan Pelanggan</title>
...
<body style="background:#428bca;">
	...
	<div class="row">
		...
		<?php
			...
			$permintaankeluhan = $_POST['permintaankeluhan'];
			$permasalahankeluhan = $_POST['permasalahankeluhan'];
			$submit = $_POST['submit'];
			$query = "SELECT max(id_keluhan) as id_keluhan FROM keluhan_pelanggan";
			$sql = mysql_query($query);
			$row= mysql_fetch_array($sql);
			$idkeluhan	= $row['id_keluhan'];
			$no = (int) substr($idkeluhan, 6, 2);
			$no++;
			$idkeluhan=date('Y').'KP'.sprintf("%02s", $no);
			
			
			$q = mysql_query("SELECT * FROM pelanggan where username = '$_SESSION[nama_pelanggan]'");
			$r = mysql_fetch_array($q);
			if(!empty($submit)){
				 $simpan = mysql_query("INSERT INTO keluhan_pelanggan VALUES('$idkeluhan', '$r[id_pelanggan]', '$permintaankeluhan', '$permasalahankeluhan', now())");
				 if($simpan==true) {
		...
		<div class="col-md-4">
			...
			<form action="" method="POST">
				...
				<div class="form-group">
					...
					<input type="text" name="namapel" id="namapel" class="form-control" value="<?php echo $r['nama']; ?>" disabled="disabled">
					<!--
				...
				<div class="form-group">
					...
					<input type="text" name="permintaankeluhan" id="permintaankeluhan" class="form-control" placeholder="Permintaan yg Tidak Bisa Dipenuhi" required>
				</div>
				<div class="form-group">
					<label>Permasalahan</label>
					<textarea  name="permasalahankeluhan" id="permasalahankeluhan" class="form-control" rows="4" placeholder="Permasalahan" required></textarea>
				</div>
